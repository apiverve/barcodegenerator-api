...
class BarcodegeneratorAPIClientError(Exception):
...
class ValidationError(BarcodegeneratorAPIClientError):
...
class BarcodegeneratorAPIClient:
    ...
    VALIDATION_RULES = {"data": {"type": "string", "required": True}, "type": {"type": "string", "required": True}, "lineColor": {"type": "string", "required": False}, "backgroundColor": {"type": "string", "required": False}, "displayValue": {"type": "boolean", "required": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the BarcodegeneratorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises BarcodegeneratorAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise BarcodegeneratorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[BarcodegeneratorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns BarcodeGeneratorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises BarcodegeneratorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise BarcodegeneratorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise BarcodegeneratorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise BarcodegeneratorAPIClientError(f"Request failed: {str(e)}")
